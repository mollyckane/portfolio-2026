...
// Per-project visual identity. Add entries here as each project gets its own theme.
const projectTheme = {
    fiadh: {
        heroFontClass: missyVoya.className,
        headingSize: "text-7xl",
        // CSS custom properties, taken directly from Fiadh's own design system
        vars: {
            "--fiadh-bg": "#e5e9f4",
            "--fiadh-surface": "#fffdf8",
            "--fiadh-brand": "#213a31",
            "--fiadh-text-primary": "#355c4e",
            "--fiadh-text-secondary": "#7f9488",
...
export default function ProjectPageClient({ params }) {
    ...
    return (
        <main
            className={`relative min-h-screen px-6 sm:px-10 ${theme ? theme.textClass : "bg-stone-50 text-stone-800"
                }`}
            style={theme?.vars}
        >
            {/* Themed background layer, fades in on load */}
            {theme && (
                <div
                    aria-hidden="true"
            ...
            <section className="relative z-10 mx-auto max-w-4xl py-30">
                <Link
                    href="/projects"
                ...
                <p
                    className={`project-animate project-animate-delay-1 mt-10 text-xs font-medium uppercase tracking-[0.28em] ${theme ? theme.mutedClass : "text-stone-500"
                        }`}
                >
                ...
                <div className="mt-3 flex flex-wrap items-end gap-4">
                    <h1
                        className={`project-animate project-animate-delay-2 tracking-tight transition-all duration-700 ease-out ${theme
                            ? `${showDecorative ? `${theme.heroFontClass} ${theme.headingSize}` : "text-6xl font-semibold"
                            } ${theme.headingClass}`
                    ...
                    {theme && (
                        <button
                            ...
                            aria-label={
                                showDecorative
                                    ? "Switch to legible font"
                                    : "Switch to decorative font"
                            }
                ...
                {selectedProject.link && (
                    <a
                        ...
                        className={`project-animate project-animate-delay-4 mt-8 inline-block text-sm font-semibold transition ${theme ? theme.accentText : "text-rose-600 hover:text-rose-700"
                            }`}
                    >
                        Visit project →
                    </a>
            ...
            <section
                className={`relative z-10 mx-auto max-w-4xl border-t py-20 ${theme ? theme.borderClass : "border-stone-200"}`}
            >
                <h2
                ...
                {overviewDetails[selectedProject.project] ? (
                    <div
                        className={`project-animate project-animate-delay-2 mt-10 overflow-hidden rounded-3xl border shadow-sm backdrop-blur ${theme ? `${theme.borderClass} bg-[color:var(--fiadh-surface)]/70` : "border-stone-200/80 bg-white/70"
                            }`}
                    >
                        {overviewDetails[selectedProject.project].map((item, index) => (
                            <div key={item.label}>
                                <div className="flex gap-4 px-5 py-5 sm:px-6">
                                    ...
                                    <div>
                                        <h3
                                            className={`text-sm font-semibold ${theme ? theme.mutedClass : "text-stone-800"}`}
                                        >
                                            {item.label}
                                        </h3>
                                        <p
                                            className={`mt-1 max-w-2xl text-sm leading-6 ${theme ? theme.paragraphClass : "text-stone-500"}`}
                                        >
                                            {item.text}
                                        </p>
                                    </div>
                                ...
                                {index < overviewDetails[selectedProject.project].length - 1 && (
                                    <div
                                        className={`mx-5 border-t sm:mx-6 ${theme ? theme.borderClass : "border-stone-200/80"}`}
                                    />
                                )}
                            </div>
                        ))}
                    ...
                    <p
                        className={`project-animate project-animate-delay-1 mt-6 ${theme ? theme.paragraphClass : "text-stone-600"}`}
                    >
            ...
            >
                ...
                <div
                    className={`project-animate project-animate-delay-1 mt-6 space-y-4 ${theme ? theme.paragraphClass : "text-stone-700"
                        }`}
                >
                    <p>
