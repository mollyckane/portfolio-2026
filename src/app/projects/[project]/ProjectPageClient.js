...
export default function ProjectPageClient({ params }) {
    ...
    const [projectKey, setProjectKey] = useState(null);   // the key from URL
    const [selectedProject, setSelectedProject] = useState(null);
    const [loaded, setLoaded] = useState(false);          // have we resolved params?
    ...
    // While params are still resolving: show nothing or a tiny loader
    if (!loaded) {
        return (
    ...
    // After resolving: if no match, show "not found"
    if (!selectedProject) {
        return (
